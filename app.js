...
function parseNumberBR(s) {
  if (!s) return null;
  // Aceita "1234,56" ou "1234.56" ou "1.234,56"
  const t = String(s).replace(/\./g, '').replace(',', '.');
  const n = Number(t);
  return Number.isFinite(n) ? n : null;
}
function sameValue(a, b, tol = 0.01) {
...
function toYMD(d) {
  // aceita "YYYY-MM-DD", "YYYYMMDD", "YYYY-MM-DDThh:mm..."
  if (!d) return '';
...
function extractParty(inf, which){
  ...
  const uf   = end ? findPathLocal(end, ['UF'])?.textContent : '';
  return { name: (name?.textContent||'—'), doc: digits(docn?.textContent||''), uf: (uf||'').trim().toUpperCase() };
}
...
async function collectXMLs(fileList){
  ...
  for (const it of items){
    try {
      ...
      if ((b.mod||'') === '55') {
        out.push({ ...b, filename: it.name, raw: it.text });
      }
    } catch (e) {
...
// ---- SINTEGRA (Registro 50) ----
// Tenta parser delimitado por ; , ou |
// Esperado: 50;CNPJ;IE;UF;DATA(YYYYMMDD);MODELO;SERIE;NUM;...;VALOR;...
...
// Lê .txt e .zip com .txt (Sintegra)
async function collectSintegra(fileList){
  const texts = [];
  for (const file of fileList){
    const low = file.name.toLowerCase();
    if (low.endsWith('.txt')) {
      ...
      for (const entry of Object.values(zip.files)){
        if (!entry.dir && entry.name.toLowerCase().endsWith('.txt')){
          const txt = await entry.async('string');
          texts.push({name: entry.name.split('/').pop(), text: txt});
        }
      }
    }
  }
  ...
  for (const t of texts){
    ...
    for (const line of lines){
      try{
        const rec = parseSintegraLine(line.trim());
        if (rec && rec.mod === '55'){
          out.push({ ...rec, filename: t.name });
        }
      }catch(e){
...
// ---- Relatório PDF (somente diferentes, sem chave) ----
...
<!doctype html><html><head><meta charset="utf-8">
  ...
  <div class="meta">
    ...
    Gerado em: ${new Date().toLocaleString('pt-BR')} • Total de notas diferentes: <strong>${rows.length}</strong>
  </div>
...
<body>
  ...
  <div class="grid2">
    <div class="box">
      ...
      ${x.emit?.name||'—'}<br>
      Doc: ${x.emit?.doc||''} • UF: ${x.emit?.uf||''}
    </div>
    <div class="box">
      <strong>Destinatário</strong><br>
      ${x.dest?.name||'—'}<br>
      Doc: ${x.dest?.doc||''} • UF: ${x.dest?.uf||''}
    </div>
  </div>
  ...
  <div class="box">
    ...
    <table>
      <thead>
        <tr><th>#</th><th>Descrição</th><th>CFOP</th><th>Un</th><th>Qtd</th><th>Valor</th></tr>
      </thead>
      <tbody>${itens || '<tr><td colspan="6" class="c">Itens não informados no XML.</td></tr>'}</tbody>
    </table>
  </div>
...
function buildExcelSheets(common, onlyS, onlyX){
  ...
  const cols = ["origem","modelo","série","nNF","emissão","valor","emitente","emit_doc","destinatário","dest_doc"];
...
// ---- UI Upload (animação/leitura nome) ----
...
// ---- Comparação principal (SINTEGRA × XML) ----
async function compareSintegraXML(filesS, filesX){
  const warn = el('warn'); warn.textContent = "";
  el('progressBar').classList.remove('hidden');

  const S = await collectSintegra(filesS);
  const X = await collectXMLs(filesX);

  if (S.length===0 || X.length===0){
    el('progressBar').classList.add('hidden');
    warn.textContent = "Envie arquivos dos dois lados (Sintegra e XML).";
    return null;
  }

  // Normaliza para chave de comparação (serie+nNF) e atributos
  const normFromS = s => ({
    origem: 'SINTEGRA',
    serie: normSerie(s.serie),
    nNF:   normNF(s.nNF),
    data:  /^\d{8}$/.test(s.data)? s.data : toYMD(s.data),
    vNF:   s.vNF,
    emit:  s.emit,
    dest:  s.dest,
    raw: s
  ...
  const SN = S.map(normFromS);
  const XN = X.filter(xx => xx.mod==='55').map(normFromX);

  const key = (r)=> `${r.serie}|${r.nNF}`;
  const mapS = new Map(), mapX = new Map();
  for (const r of SN) if (r.serie && r.nNF) mapS.set(key(r), r);
  ...
  // Encontra comuns (mesma série+NF e MESMO valor e MESMA data, tolerância em valor)
  for (const [k, s] of mapS){
    const x = mapX.get(k);
    if (!x) continue;
    const sameDate = (s.data && x.data) ? (s.data === x.data) : true; // se faltar data, não bloqueia
  ...
  el('stats').innerHTML = `
    <div>Registros SINTEGRA<br><strong>${mapS.size}</strong></div>
    <div>Registros XML (mod 55)<br><strong>${mapX.size}</strong></div>
    <div>Em comum<br><strong>${common.length}</strong></div>
    <div>Diferenças<br><strong>${onlyS.length + onlyX.length}</strong></div>
  `;

  const headersCommon = ["Série","NF","Emissão","Valor","Emitente","Destinatário"];
  const rowsCommon = common.map(r=>[
    r.serie, r.nNF, ymdToBR(r.data||''), moneyBR(r.vNF||''),
    r.emit?.name || '—', r.dest?.name || '—'
  ]);
  el('tableCommon').innerHTML = tableFromRows(headersCommon, rowsCommon);

  const headersOnly = ["Origem","Série","NF","Emissão","Valor"];
  el('tableOnlyS').innerHTML = tableFromRows(headersOnly, onlyS.map(r=>[
    "SINTEGRA", r.serie, r.nNF, ymdToBR(r.data||''), moneyBR(r.vNF||'')
  ]));
  el('tableOnlyX').innerHTML = tableFromRows(headersOnly, onlyX.map(r=>[
    "XML", r.serie, r.nNF, ymdToBR(r.data||''), moneyBR(r.vNF||'')
  ]));

  // Ações
  const dlExcel = el('dlExcel');
  const dlPDF   = el('dlPDF');
  const viewDiffBtn = el('viewDiffBtn');

  dlExcel.disabled = dlPDF.disabled = viewDiffBtn.disabled = false;
  ...
  dlPDF.onclick = ()=>{
    // PDF apenas com diferentes (sem chave)
    const diffs = [
      ...onlyS.map(r=>({ ...r, origem: 'SÓ SINTEGRA' })),
      ...onlyX.map(r=>({ ...r, origem: 'SÓ XML' }))
    ];
    // tenta pegar CNPJ cliente do lado XML (dest)
    const meta = {};
    const anyX = XN.find(Boolean);
    if (anyX) { meta.companyName = anyX.dest?.name; meta.companyDoc = anyX.dest?.doc; }
    openClientPDFPreview(meta, diffs);
  };

  viewDiffBtn.onclick = ()=>{
    showDanfeModal(onlyX, X); // somente as que vieram de XML
  };

  // Guarda contexto
  window.__cmp = { S, X, onlyS, onlyX, common, mapS, mapX };

  el('progressBar').classList.add('hidden');
  return { common, onlyS, onlyX };
}

// ---- Modal DANFE (somente diferenças do lado XML) ----
function danfeCardHTML(x){
  return `
    <div class="danfe-card" style="border:1px solid #ddd;border-radius:8px;padding:10px;margin:8px 0;">
      <h4 style="margin:0 0 6px 0;">NF ${x.nNF} — Série ${x.serie}</h4>
...
function showDanfeModal(onlyXNorm, XRaw){
  ...
  // faz um map da key->obj xml bruto para abrir itens/itens
  const byKey = new Map();
  ...
  listEl.querySelectorAll('button[data-key]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      const rec = onlyXNorm.find(o => (o.raw.key === key || o.raw.key === key));
      const full = rec?.__full || (window.__cmp?.X || []).find(xx=>xx.key===key);
      if (full) openDANFEWindow(full);
    });
  });

  el('closeDanfe').onclick = ()=> modal.classList.add('hidden');
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  const filesS = el('filesS');
  const filesX = el('filesX');
  const compareBtn = el('compareBtn');
  const resetBtn = el('resetBtn');

  wireUploadUI(document.getElementById('boxS'), filesS, 'SINTEGRA (.txt/.zip)');
  wireUploadUI(document.getElementById('boxX'), filesX, 'XML/ZIP (NF-e)');

  compareBtn.addEventListener('click', async ()=>{
    compareBtn.disabled = true; compareBtn.textContent = "Comparando...";
    try { await compareSintegraXML(filesS.files, filesX.files); }
  ...
  resetBtn.addEventListener('click', ()=>{
    filesS.value = ""; filesX.value = "";
    el('warn').textContent = "";
    el('summary').classList.add('hidden');
    ['boxS','boxX'].forEach(id=>{
      const labelEl = document.getElementById(id);
      const span = labelEl.querySelector('span');
      labelEl.classList.remove('filled','dragover');
      span.innerHTML = (id==='boxS')
        ? '<strong>SINTEGRA (.txt/.zip)</strong><br><small>Arraste aqui ou clique</small>'
        : '<strong>XML/ZIP (NF-e)</strong><br><small>Arraste aqui ou clique</small>';
    });
  });
});
