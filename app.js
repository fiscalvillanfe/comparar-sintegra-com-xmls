// ====== CONFIGURAÇÃO DE MARCA ======
const BRAND_DATA_URL = "data:image/png;base64,"; // troque pelo seu data URL

// ====== UTIL ======
const $ = (id)=>document.getElementById(id);
const digits = s => (s||"").replace(/\D+/g,"");
const moneyBR = v => {
  const n = (typeof v==="number") ? v : Number(String(v).replace(/\./g,"").replace(",","."));
  return Number.isFinite(n) ? n.toLocaleString("pt-BR",{style:"currency",currency:"BRL"}) : (v??"");
};
const parseNumberBR = s => {
  if (s==null||s==="") return null;
  const n = Number(String(s).replace(/\./g,"").replace(",","."));
  return Number.isFinite(n) ? n : null;
};
const ymdToBR = ymd => (ymd && /^\d{8}$/.test(ymd)) ? `${ymd.slice(6,8)}/${ymd.slice(4,6)}/${ymd.slice(0,4)}` : ymd||"";
const table = (head, rows) => {
  const th = `<thead><tr>${head.map(h=>`<th>${h}</th>`).join("")}</tr></thead>`;
  const tb = `<tbody>${
    rows.length? rows.map(r=>`<tr>${r.map(c=>`<td>${c??""}</td>`).join("")}</tr>`).join("")
               : `<tr><td colspan="${head.length}">Sem registros.</td></tr>`}</tbody>`;
  return `<div class="tablewrap"><table>${th}${tb}</table></div>`;
};

// ====== VALIDAÇÃO SINTEGRA ======
const isValidYMD = (s) => {
  if (!/^\d{8}$/.test(s)) return false;
  const y = +s.slice(0,4), m = +s.slice(4,6), d = +s.slice(6,8);
  if (y < 2000 || y > 2100) return false;
  if (m < 1 || m > 12) return false;
  const md=[31,(y%4===0&&(y%100!==0||y%400===0))?29:28,31,30,31,30,31,31,30,31,30,31];
  return d>=1 && d<=md[m-1];
};
const pickYMD  = line => (line.match(/\b\d{8}\b/g)||[]).find(isValidYMD) || "";
const pickCFOP = line => { const m=line.match(/\b([1256]\d{3})\b/g); return m? m[m.length-1].slice(0,4):""; };
const pickMoney= line => { const m=line.match(/(\d{1,3}(\.\d{3})*,\d{2}|\d+,\d{2}|\d+\.\d{2})/g); return m? m[m.length-1]:""; };
const pickNF   = line => { const m=line.match(/\b\d{1,9}\b/g); return m? m[m.length-1]:""; };
const pickSerie= line => { const m=line.match(/\b\d{1,3}\b/g); return m? m[0]:"1"; };

// ====== XML ======
const parseXML = txt => new DOMParser().parseFromString(txt,"text/xml");
const byName = (doc, name)=>Array.from(doc.getElementsByTagName("*")).filter(n=>(n.localName||n.nodeName)===name);
const first = (doc,name)=>byName(doc,name)[0]||null;
const findPath = (root, arr) => {
  if (!root) return null; let cur=root;
  for (const k of arr){ cur = Array.from(cur.children).find(ch => (ch.localName||ch.nodeName)===k); if (!cur) return null; }
  return cur;
};
const pickInfNFe = (doc)=> first(doc,"infNFe") || first(first(doc,"NFe"),"infNFe") || doc.documentElement;
const getAccessKey = (doc)=>{
  const infProt = first(doc,"infProt");
  if (infProt){ const c=findPath(infProt,["chNFe"]); if (c?.textContent) return digits(c.textContent); }
  const inf = pickInfNFe(doc);
  const id = inf?.getAttribute?.("Id");
  return (id && id.toUpperCase().startsWith("NFE")) ? digits(id) : "";
};
const extractParty = (inf, which)=>{
  const base = which==="dest"?"dest":"emit";
  const name = findPath(inf,[base,"xNome"])?.textContent || "—";
  const docn = findPath(inf,[base,"CNPJ"]) || findPath(inf,[base,"CPF"]);
  const doc = digits(docn?.textContent || "");
  const end  = findPath(inf,[base, "ender"+(base==="emit"?"Emit":"Dest")]);
  const uf   = (end? findPath(end,["UF"])?.textContent:""||"").trim().toUpperCase();
  return {name, doc, uf};
};
const extractItems = (inf)=>{
  const dets = byName(inf,"det");
  return dets.map(d=>{
    const prod = findPath(d,["prod"]);
    return {
      xProd: findPath(prod,["xProd"])?.textContent || "",
      CFOP : findPath(prod,["CFOP"])?.textContent || "",
      uCom : findPath(prod,["uCom"])?.textContent || "",
      qCom : findPath(prod,["qCom"])?.textContent || "",
      vProd: findPath(prod,["vProd"])?.textContent || ""
    };
  });
};
const sumVProd = items => items.reduce((s,it)=>{
  const n = parseNumberBR(it.vProd); return s + (Number.isFinite(n)? n:0);
},0);
const extractXMLBasics = (txt)=>{
  const doc = parseXML(txt);
  const inf = pickInfNFe(doc);
  const key  = getAccessKey(doc);
  const mod  = findPath(inf,["ide","mod"])?.textContent || "";
  const nNF  = findPath(inf,["ide","nNF"])?.textContent || "";
  const serie= findPath(inf,["ide","serie"])?.textContent || "";
  const dhEmi= (findPath(inf,["ide","dhEmi"])?.textContent || findPath(inf,["ide","dEmi"])?.textContent || "");
  const vNF  = findPath(inf,["total","ICMSTot","vNF"])?.textContent || "";
  const emit = extractParty(inf,"emit");
  const dest = extractParty(inf,"dest");
  const items= extractItems(inf);
  const vItens = sumVProd(items);
  return {key,mod,nNF,serie,dhEmi,vNF,vItens,emit,dest,dets:items};
};
async function readXMLs(fileList){
  const raws=[];
  for (const f of fileList){
    const name=f.name.toLowerCase();
    if (name.endsWith(".xml")){
      raws.push({name:f.name, text:await f.text()});
    } else if (name.endsWith(".zip")){
      const zip=await JSZip.loadAsync(f);
      for (const e of Object.values(zip.files)){
        if (!e.dir && e.name.toLowerCase().endsWith(".xml")){
          const txt=await e.async("string");
          raws.push({name:e.name.split("/").pop(), text:txt});
        }
      }
    }
  }
  const out=[];
  for (const r of raws){
    try{ const b=extractXMLBasics(r.text); if (b.mod==="55") out.push({...b, filename:r.name}); }
    catch(e){ console.warn("XML ignorado:", r.name, e); }
  }
  return out;
}
const guessCompanyDoc = (xmls)=>{
  const c=new Map();
  for (const x of xmls){ if (x.emit?.doc) c.set(x.emit.doc,(c.get(x.emit.doc)||0)+1);
                         if (x.dest?.doc) c.set(x.dest.doc,(c.get(x.dest.doc)||0)+1); }
  let best="",m=-1; for (const [k,v] of c) if (v>m){ best=k; m=v; }
  return best;
};
const tagXmlDirection = (xmls, cnpjEmpresa)=>{
  for (const x of xmls){
    x._tipo = (x.emit?.doc===cnpjEmpresa)? "SAÍDA" :
              (x.dest?.doc===cnpjEmpresa)? "ENTRADA" : "DESCONHECIDO";
  }
  return xmls;
};

// ====== SINTEGRA ======
function parseSintegra(txt){
  const lines = txt.split(/\r?\n/).map(l=>l.trim()).filter(Boolean);
  let cnpjInformante="";
  const itens=[];

  for (const ln of lines){
    if (ln.startsWith("10")){
      const m = ln.match(/(\d{14})/);
      if (m) cnpjInformante = m[1];
      continue;
    }
    if (!ln.startsWith("50")) continue;

    const delim = [";","|",","].find(d=>ln.includes(d));
    let campos = null;
    if (delim) campos = ln.split(delim).map(s=>s.trim());

    let cnpjContrap="", data="", mod="55", serie="", nNF="", vItem="", cfop="";

    if (campos){
      const cnpjs = campos.map(c=>digits(c)).filter(c=>/^\d{14}$/.test(c));
      cnpjContrap = cnpjs.find(c=>c!==cnpjInformante) || cnpjs[0] || "";
      data  = (campos.find(c=>/^\d{8}$/.test(c) && isValidYMD(c))) || pickYMD(ln) || "";
      serie = (campos.find(c=>/^\d{1,3}$/.test(c))) || pickSerie(ln) || "1";
      nNF   = (campos.find(c=>/^\d{1,9}$/.test(c))) || pickNF(ln) || "";
      const cf = campos.find(c=>/^[1256]\d{3}$/.test(c));
      cfop  = cf ? cf.slice(0,4) : pickCFOP(ln);
      const vals = campos.filter(c=>/(\d{1,3}(\.\d{3})*,\d{2}|\d+,\d{2}|\d+\.\d{2})/.test(c));
      vItem = vals.length ? vals[vals.length-1] : pickMoney(ln);
    } else {
      const cnpjs = ln.match(/\d{14}/g) || [];
      cnpjContrap = cnpjs.find(c=>c!==cnpjInformante) || cnpjs[0] || "";
      data  = pickYMD(ln);
      serie = pickSerie(ln);
      nNF   = pickNF(ln);
      cfop  = pickCFOP(ln);
      vItem = pickMoney(ln);
    }

    serie = String(Number(digits(serie)||0));
    nNF   = String(Number(digits(nNF)||0));
    if (!isValidYMD(data)) data = "";
    if (!/^\d{4}$/.test(cfop)) cfop = "";

    itens.push({ cnpjContrap:digits(cnpjContrap), data, mod, serie, nNF, vItem, cfop });
  }

  // agrega por NF
  const keyNF = r => `${r.cnpjContrap}|${r.mod}|${r.serie}|${r.nNF}|${r.data}`;
  const map = new Map();
  for (const it of itens){
    if (it.mod!=="55") continue;
    if (!it.cnpjContrap || !it.nNF || !it.serie || !it.data) continue;
    const k = keyNF(it);
    const prev = map.get(k) || { ...it, vItens:0, cfops:{} };
    const n = parseNumberBR(it.vItem);
    if (Number.isFinite(n)) prev.vItens += n;
    if (it.cfop) prev.cfops[it.cfop]=(prev.cfops[it.cfop]||0)+1;
    map.set(k, prev);
  }
  const dirByCFOP = cf => /^1|^2/.test(cf) ? "ENTRADA" : /^5|^6/.test(cf) ? "SAÍDA" : "DESCONHECIDO";
  const rows = [...map.values()].map(r=>{
    const cf = Object.entries(r.cfops).sort((a,b)=>b[1]-a[1])[0]?.[0] || "";
    return { origem:"SINTEGRA", cnpjContrap:r.cnpjContrap, mod:r.mod, serie:r.serie, nNF:r.nNF, data:r.data, vItens:r.vItens, _tipo:dirByCFOP(cf) };
  });
  return { cnpjInformante, rows };
}

// ====== NORMALIZAÇÃO E COMPARAÇÃO ======
const adaptiveKey = r =>
  (r.origem==="SINTEGRA")
    ? `${r.cnpjContrap}|${r.mod}|${r.serie}|${r.nNF}|${r.data}`
    : `${r._cnpjContrap||""}|${r.mod||"55"}|${Number(digits(r.serie)||0)}|${Number(digits(r.nNF)||0)}|${(r.data||"")}`;

const buildXMLNorm = (xmls, cnpjEmpresa, dirSel) => {
  const out=[];
  for (const x of xmls){
    if (dirSel!=="AUTO" && x._tipo!==dirSel) continue;
    const cnpjContrap = (x._tipo==="SAÍDA") ? x.dest?.doc : (x._tipo==="ENTRADA") ? x.emit?.doc : "";
    out.push({
      origem:"XML",
      mod:x.mod||"55",
      serie:String(Number(digits(x.serie)||0)),
      nNF:String(Number(digits(x.nNF)||0)),
      data:(x.dhEmi? x.dhEmi.slice(0,10).replaceAll("-","") : ""),
      vItens:x.vItens,
      vNF:x.vNF,
      emit:x.emit, dest:x.dest,
      _tipo:x._tipo,
      _cnpjContrap:cnpjContrap||"",
      key:x.key, raw:x
    });
  }
  return out;
};
const buildSintegraNorm = (rows, dirSel) =>
  rows.filter(r=>dirSel==="AUTO" || r._tipo===dirSel);

const compare = (SN, XN) =>{
  const mS = new Map(), mX = new Map();
  for (const s of SN) mS.set(adaptiveKey(s), s);
  for (const x of XN) mX.set(adaptiveKey(x), x);

  const common=[], onlyS=[], onlyX=[];
  for (const [k,s] of mS){
    const x=mX.get(k);
    if (x){
      const obs = (Math.abs((parseNumberBR(s.vItens)||0)-(parseNumberBR(x.vItens)||0))>0.05)
        ? `Valor itens difere (SINTEGRA ${moneyBR(s.vItens)} × XML ${moneyBR(x.vItens)})` : "";
      common.push({ serie:s.serie||x.serie, nNF:s.nNF||x.nNF, data:s.data||x.data, vItens:x.vItens, emit:x.emit, dest:x.dest, _tipo:x._tipo, _obs:obs });
    } else {
      onlyS.push(s);
    }
  }
  for (const [k,x] of mX) if (!mS.has(k)) onlyX.push(x);
  return {common, onlyS, onlyX};
};

// ====== PDF ======
function openPDF(meta, diffs){
  const css = `
    @page{margin:16mm} body{font-family:Segoe UI,Roboto,Arial,sans-serif;color:#222}
    header{display:flex;align-items:center;gap:8px;margin-bottom:10px}
    header img{width:34px;height:34px;border-radius:8px}
    header .t{color:#8B0000;font-weight:800}
    table{width:100%;border-collapse:collapse;font-size:12px}
    thead{display:table-header-group}
    th,td{border-bottom:1px solid #eee;padding:6px 6px;text-align:left}
    th{background:#8B0000;color:#fff}
    td.num{text-align:right}
  `;
  const head = ["Origem","NF","Série","Emissão","Valor (R$)","Emitente","CNPJ","Destinatário","CNPJ"];
  const rows = diffs.map(r=>`
    <tr>
      <td>${r.origem}</td>
      <td>${r.nNF||""}</td>
      <td>${r.serie||""}</td>
      <td>${ymdToBR(r.data||"")}</td>
      <td class="num">${moneyBR(r.vItens||r.vNF||"")}</td>
      <td>${r.emit?.name||"—"}</td>
      <td>${r.emit?.doc||""}</td>
      <td>${r.dest?.name||"—"}</td>
      <td>${r.dest?.doc||""}</td>
    </tr>`).join("");

  const html = `<!doctype html><html><head><meta charset="utf-8"><style>${css}</style><title>Relatório</title></head>
  <body>
    <header><img src="${BRAND_DATA_URL}"><div class="t">Relatório — Notas Diferentes</div></header>
    <div style="font-size:12px;color:#666;margin-bottom:8px">
      Empresa: <strong>${meta.name||"—"}</strong> — ${meta.doc||""} • Gerado em ${new Date().toLocaleString("pt-BR")}
    </div>
    <table><thead><tr>${head.map(h=>`<th>${h}</th>`).join("")}</tr></thead>
      <tbody>${rows || `<tr><td colspan="9">Sem diferenças.</td></tr>`}</tbody></table>
    <script>setTimeout(()=>{try{window.print()}catch(e){}},400)</script>
  </body></html>`;
  const w=window.open("","_blank"); if(!w){alert("Permita pop-ups.");return;}
  w.document.open(); w.document.write(html); w.document.close();
}

// ====== DANFE (about:blank) ======
function danfeHTML(x){
  const chave = digits(x.key||""); const chaveFmt = chave? chave.replace(/(\d{4})(?=\d)/g,"$1 ").trim() : "—";
  const css = `
    @page{size:A4;margin:12mm}
    body{font-family:Segoe UI,Roboto,Arial,sans-serif}
    .box{border:1px solid #000;border-radius:4px;padding:6px;margin-bottom:6px}
    .g2{display:grid;grid-template-columns:1fr 1fr;gap:6px}
    .g3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px}
    table{width:100%;border-collapse:collapse;font-size:12px}
    th,td{border:1px solid #000;padding:4px 6px} th{background:#eee}
  `;
  const its=(x.dets||[]).map((d,i)=>`
    <tr><td>${i+1}</td><td>${d.xProd||""}</td><td>${d.CFOP||""}</td><td>${d.uCom||""}</td><td>${d.qCom||""}</td><td>${moneyBR(d.vProd||"")}</td></tr>`).join("");
  return `<!doctype html><html><head><meta charset="utf-8"><style>${css}</style><title>DANFE</title></head>
  <body>
    <div class="box"><strong>DANFE — NF ${x.nNF||""} • Série ${x.serie||""} • Emissão ${ymdToBR((x.dhEmi||"").slice(0,10).replaceAll("-",""))}</strong></div>
    <div class="box"><strong>Chave:</strong> ${chaveFmt}</div>
    <div class="g2">
      <div class="box"><strong>Emitente</strong><br>${x.emit?.name||"—"} • Doc ${x.emit?.doc||""}</div>
      <div class="box"><strong>Destinatário</strong><br>${x.dest?.name||"—"} • Doc ${x.dest?.doc||""}</div>
    </div>
    <div class="g3">
      <div class="box"><strong>Número</strong><br>${x.nNF||""}</div>
      <div class="box"><strong>Série</strong><br>${x.serie||""}</div>
      <div class="box"><strong>Valor Total</strong><br>${moneyBR(x.vNF||"")}</div>
    </div>
    <div class="box">
      <strong>Itens</strong>
      <table><thead><tr><th>#</th><th>Descrição</th><th>CFOP</th><th>Un</th><th>Qtd</th><th>Valor</th></tr></thead>
      <tbody>${its || '<tr><td colspan="6">Sem itens no XML.</td></tr>'}</tbody></table>
    </div>
    <script>setTimeout(()=>{try{window.print()}catch(e){}},400)</script>
  </body></html>`;
}
function openDANFE(fullXML){
  const w=window.open("","_blank"); if(!w){alert("Permita pop-ups.");return;}
  w.document.open(); w.document.write(danfeHTML(fullXML)); w.document.close();
}

// ====== UI ======
function setBrand(){ const f=$("favicon"), l=$("logo"); if(f) f.href=BRAND_DATA_URL; if(l) l.src=BRAND_DATA_URL; }
function wireUploader(box, input, title){
  const refresh=()=>{
    const s = box.querySelector("span");
    const files = Array.from(input.files||[]);
    if(!files.length){ s.innerHTML=`<strong>${title}</strong><br><small>Arraste aqui ou clique</small>`; return; }
    const first = files[0].name; const rest=files.length>1?` <span class="count">+${files.length-1}</span>`:"";
    s.innerHTML = `<strong>${title}</strong><br><small>${files.length} arquivo(s)</small><div style="margin-top:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${first}${rest}</div>`;
  };
  input.addEventListener("change", refresh);
  ["dragenter","dragover"].forEach(ev=>box.addEventListener(ev,e=>{e.preventDefault();box.classList.add("drag")}));
  ["dragleave","drop"].forEach(ev=>box.addEventListener(ev,e=>{e.preventDefault();box.classList.remove("drag")}));
  box.addEventListener("drop", e=>{ const dt=e.dataTransfer; if(dt?.files?.length){ input.files=dt.files; input.dispatchEvent(new Event("change")); }});
}

function fillTables(common, onlyS, onlyX){
  $("tCommon").innerHTML  = table(["Tipo","Série","NF","Emissão","Valor Itens","Emitente","Destinatário","Obs"],
    common.map(r=>[r._tipo||"—",r.serie,r.nNF,ymdToBR(r.data),moneyBR(r.vItens),r.emit?.name||"—",r.dest?.name||"—",r._obs||""]));
  $("tOnlyS").innerHTML   = table(["Origem","Tipo","Série","NF","Emissão","Valor Itens"],
    onlyS.map(r=>["SINTEGRA",r._tipo||"—",r.serie,r.nNF,ymdToBR(r.data),moneyBR(r.vItens)]));
  $("tOnlyX").innerHTML   = table(["Origem","Tipo","Série","NF","Emissão","Valor Itens"],
    onlyX.map(r=>["XML",r._tipo||"—",r.serie,r.nNF,ymdToBR(r.data),moneyBR(r.vItens)]));
}

function showDanfeModal(xmlDiffs, allXML){
  const byNS=new Map(); for(const x of allXML){ byNS.set(`${Number(digits(x.serie)||0)}|${Number(digits(x.nNF)||0)}`,x); }
  const list = xmlDiffs.map(nx=>{
    const full = byNS.get(`${nx.serie}|${nx.nNF}`) || nx.raw || nx;
    nx.__full = full;
    return `<div class="dcard">
      <div><strong>NF ${full.nNF}</strong> • Série ${full.serie} • Emissão ${ymdToBR((full.dhEmi||"").slice(0,10).replaceAll("-",""))}</div>
      <div style="font-size:13px;color:#555">Emitente: ${full.emit?.name||"—"} • Destinatário: ${full.dest?.name||"—"}</div>
      <div class="row" style="margin-top:6px"><button class="btn primary" data-nf="${full.nNF}" data-se="${full.serie}">Abrir DANFE</button></div>
    </div>`;
  }).join("");
  $("danfeList").innerHTML = list || "Sem notas do lado XML.";
  $("danfeModal").classList.remove("hidden");
  $("danfeList").querySelectorAll("button[data-nf]").forEach(b=>{
    b.onclick=()=>{
      const nf=b.getAttribute("data-nf"), se=b.getAttribute("data-se");
      const rec = xmlDiffs.find(o=>o.nNF===nf && o.serie===se);
      openDANFE(rec?.__full || rec?.raw || rec);
    };
  });
  $("closeDanfe").onclick = ()=>$("danfeModal").classList.add("hidden");
  $("danfeModal").onclick = (e)=>{ if(e.target.id==="danfeModal") $("danfeModal").classList.add("hidden"); };
}

// ====== FLUXO PRINCIPAL ======
async function run(){
  $("msg").textContent=""; $("bar").classList.remove("hidden");
  try{
    const sinFile = $("sintegra").files[0] || null;
    const xmlFiles= $("xmls").files || [];
    const dirSel  = $("dirSel").value;

    const XML = await readXMLs(xmlFiles);
    const companyGuess = guessCompanyDoc(XML);

    // Sintegra
    let SHeader=null, SRows=[];
    if (sinFile){
      const parsed = parseSintegra(await sinFile.text());
      SHeader = {cnpj: parsed.cnpjInformante || ""};
      SRows   = parsed.rows;
    }
    const companyDoc = digits(SHeader?.cnpj || "") || companyGuess || "";
    tagXmlDirection(XML, companyDoc);

    const XN = buildXMLNorm(XML, companyDoc, dirSel);
    const SN = buildSintegraNorm(SRows, dirSel);

    const {common, onlyS, onlyX} = compare(SN, XN);

    $("out").classList.remove("hidden");
    $("stats").innerHTML = `
      <div>XMLs (mod 55): <br><strong>${XN.length}</strong></div>
      <div>SINTEGRA (NF agregada): <br><strong>${SN.length}</strong></div>
      <div>Em comum: <br><strong>${common.length}</strong></div>
      <div>Só Sintegra: <br><strong>${onlyS.length}</strong></div>
      <div>Só XML: <br><strong>${onlyX.length}</strong></div>
      <div>CNPJ Empresa: <br><strong>${companyDoc||"—"}</strong></div>
    `;
    fillTables(common, onlyS, onlyX);

    // Ações
    const btnPDF = $("pdf"), btnDanfe = $("danfe");
    btnPDF.disabled = false;
    btnPDF.onclick = ()=>{
      const diffs = [
        ...onlyS.map(r=>({ ...r, origem:"SÓ SINTEGRA" })),
        ...onlyX.map(r=>({ ...r, origem:"SÓ XML" }))
      ];
      const any = XML.find(xx => (xx.emit?.doc===companyDoc) || (xx.dest?.doc===companyDoc)) || XML[0];
      const meta = { name: (any && (any.emit?.doc===companyDoc ? any.emit?.name : any.dest?.name)) || "", doc: companyDoc };
      openPDF(meta, diffs);
    };

    btnDanfe.disabled = onlyX.length===0;
    btnDanfe.onclick = ()=> showDanfeModal(onlyX, XML);

    window.__cmp = { XML, SN, XN, common, onlyS, onlyX, companyDoc };
  } catch(e){
    console.error(e); $("msg").textContent = e?.message || "Erro ao comparar.";
  } finally {
    $("bar").classList.add("hidden");
  }
}

// ====== BOOT ======
document.addEventListener("DOMContentLoaded", ()=>{
  setBrand();
  wireUploader($("boxS"), $("sintegra"), "SINTEGRA (.txt)");
  wireUploader($("boxX"), $("xmls"), "XMLs / ZIPs");
  $("go").onclick = ()=>{ $("go").disabled=true; $("go").textContent="Comparando..."; run().finally(()=>{ $("go").disabled=false; $("go").textContent="Comparar"; }); };
  $("clear").onclick = ()=>{
    $("sintegra").value=""; $("xmls").value="";
    $("out").classList.add("hidden"); $("msg").textContent="";
    $("boxS").querySelector("span").innerHTML="<strong>SINTEGRA (.txt)</strong><br><small>Arraste aqui ou clique</small>";
    $("boxX").querySelector("span").innerHTML="<strong>XMLs / ZIPs</strong><br><small>Entradas e saídas</small>";
  };
});
